...
暗黙キャストが、メモリ侵犯の虞を考慮し、昇格を除き、暗黙キャストを提供しない。

Int 型から Num 型、Real 型から Complex 型、String 型から Chd 型、Char 型から String 型などのように、情報及び精度を失わせない場合は、明示キャストを要せず、暗黙的にキャストが行われる。この暗黙的キャストを昇格という。
